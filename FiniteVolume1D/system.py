...
class System:
    ...
    def set_boundary_condition(self):
        if self.boundary_condition == "reflective":
            self._set_reflective_boundary_condition(self.velocity, self.pressure, self.density)
        else:
            raise ValueError(
    ...
    @staticmethod
    def _set_reflective_boundary_condition(velocity: np.ndarray, pressure: np.ndarray, density: np.ndarray):

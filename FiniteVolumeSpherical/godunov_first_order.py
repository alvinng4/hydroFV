...
def solving_step(system: System, dt: float, solver) -> None:
    ...
    # Get the fluxes
    flux_mass = density_sol * velocity_sol * (1.0 + 2.0 * dr_arr / system.mid_points[:-1])
    flux_momentum = density_sol * (velocity_sol * velocity_sol) * (1.0 + 2.0 * dr_arr / system.mid_points[:-1]) 
    flux_momentum += pressure_sol
    flux_energy = (
        pressure_sol * (system.gamma / (system.gamma - 1.0))
        + 0.5 * density_sol * (velocity_sol * velocity_sol)
    ) * velocity_sol * (1.0 + 2.0 * dr_arr / system.mid_points[:-1])
